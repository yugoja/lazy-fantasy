...
### 2. `[~]` No migration runner / version tracking — **Alembic landed locally; prod/staging stamp pending**
...
**Remaining.** Run `alembic stamp head` on staging (verify app still boots, `SELECT version_num FROM alembic_version` returns `a1c99c58f11a`), then on prod (after `pg_dump` backup). Then this item flips to `[x]`.
