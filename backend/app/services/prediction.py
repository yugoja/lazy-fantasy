...
def validate_prediction_players(
    ...
    either_team_player_ids: list[int],
) -> tuple[bool, str]:
    """
    Validate that predicted players belong to the correct teams.

    - team1_player_ids must all belong to match.team_1_id
    - team2_player_ids must all belong to match.team_2_id
    - either_team_player_ids can belong to either team
    ...
    for player_id in team1_player_ids:
        ...
        if player.team_id != match.team_1_id:
            return False, f"Player {player.name} must belong to {match.team_1.name}"
    ...
    for player_id in team2_player_ids:
        ...
        if player.team_id != match.team_2_id:
            return False, f"Player {player.name} must belong to {match.team_2.name}"

    valid_team_ids = {match.team_1_id, match.team_2_id}
    for player_id in either_team_player_ids:
        player = db.query(Player).filter(Player.id == player_id).first()
        if not player:
            return False, f"Player with ID {player_id} not found"
...
def create_prediction(
    ...
    prediction = Prediction(
        ...
        predicted_most_runs_team1_player_id=predicted_most_runs_team1_player_id,
        predicted_most_runs_team2_player_id=predicted_most_runs_team2_player_id,
        predicted_most_wickets_team1_player_id=predicted_most_wickets_team1_player_id,
        predicted_most_wickets_team2_player_id=predicted_most_wickets_team2_player_id,
...
def update_prediction(
    ...
    prediction.predicted_most_runs_team1_player_id = predicted_most_runs_team1_player_id
    prediction.predicted_most_runs_team2_player_id = predicted_most_runs_team2_player_id
